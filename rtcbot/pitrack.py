...
class PiH264StreamTrack(EncodedStreamTrack):
    ...
    def __init__(self, subscription, fps=30., clock_rate=90000, startedCallback=None) -> None:
        super().__init__()
        if fps is None:
           fps = 30.
        self._bufferSubscription = subscription
        self._timestamp = 0
        self._frame_time = 1 / fps
        self._clock_rate = clock_rate
        self._startedCallBack = startedCallback
        self._startedCallBack()  # Not sure its the proper way to set it up ... but  its raedy as is
    ...
    async def recv_encoded(self, keyframe=False) -> List[bytes]:
        while True:
            buffer = await self._bufferSubscription.get()
            while len(buffer) < 64:
                buffer += await self._bufferSubscription.get()
            if (buffer[4] & 0x1F) != 0x01 or not keyframe:
                break
        self._timestamp += int(self._frame_time * self._clock_rate)
        timestamp = self._timestamp
        return self._packetize(self._split_bitstream(buffer)), timestamp
